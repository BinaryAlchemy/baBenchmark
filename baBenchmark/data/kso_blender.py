...
def enable_gpu_devices(addon_name='cycles', use_CPU=False, use_optix=False):
    ...
    if v_major > 2:
        ...
        for device in devices:
            if (device.type == 'CPU'):
                if not use_CPU:
                    log_msg(f"[GPU]     IGNORING device {device.name}")
                    continue
                device.use = True
                log_msg(f"[GPU]     Enabling device {device.name}")
            else:
                if gpu_count==0:
                    log_msg(f"[GPU]     Enabling device {gpu_count} {device.name}")
                    device.use = True
                else:
                    log_msg(f"[GPU]     IGNORING device {gpu_count} {device.name}")
                    device.use = False
...
if __name__ == "__main__":
    ...
    log_msg(" Blender version: {}.{}.{}".format(bpy.app.version[0],bpy.app.version[1],bpy.app.version[2]))
